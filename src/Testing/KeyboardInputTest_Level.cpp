#include "KeyboardInputTest_Level.h"
#include "../lib/engine/Keyboard.h"

// This level holds test code for Keyboard input. Press any key, and it should show the name of the key pressed, top-left of the console window.
...
void KeyboardInputTest_Level::start(GameLoop &gl) // Runs when the level starts
{
    gl.showStats = true;
}

void KeyboardInputTest_Level::update(GameLoop &gl) // Runs every frame while the level is loaded
{
    ...
    if (isKeyDown(Key::NUM_LOCK)) keyboardStr = "NUM_LOCK";
